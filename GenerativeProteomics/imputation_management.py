...
class Imputation_Management:
    def __init__ (self, model, df_missing, missing_file_path):
        ...
        self.dict_imputation_methods = {"GAIN_DANN_model" : hugging_face_gain_dann, "medium_imputation" : medium_imputation}
    ...
    def run_model(self, model):
        if model not in self.dict_imputation_methods:
            ...
            return self.dict_imputation_methods[model](self.df)




def hugging_face_gain_dann(dataset_path):
...
def medium_imputation(dataset):
    ...
    df[num_cols] = df[num_cols].astype(float).replace(0, np.nan)

    col_means = df[num_cols].mean().fillna(0.0)
